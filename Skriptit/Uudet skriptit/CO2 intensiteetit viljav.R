...
outersect(colnames(Grassland_korotettu_elop), colnames(Grassland_korotettu_elop_raivio))
...
outersect(colnames(Grassland_korotettu_mineraalimaa), colnames(Grassland_korotettu_mineraalimaa_raivio))
...
Cropland_mineral<-Cropland_mineral %>% select(!(ends_with(".x"))) #Ainoastaan tulokset eli ne, joilla x. tai .y päätettä ei ole. 
Cropland_mineral<-Cropland_mineral %>% select(!(ends_with(".y")))

Cropland_elop<-Cropland_elop %>% select(!(ends_with(".x"))) #Ainoastaan tulokset eli ne, joilla x. tai .y päätettä ei ole. 
Cropland_elop<-Cropland_elop %>% select(!(ends_with(".y")))

Grassland_mineral<-Grassland_mineral %>% select(!(ends_with(".x"))) #Ainoastaan tulokset eli ne, joilla x. tai .y päätettä ei ole. 
Grassland_mineral<-Grassland_mineral %>% select(!(ends_with(".y")))

Grassland_elop<-Grassland_elop %>% select(!(ends_with(".x"))) #Ainoastaan tulokset eli ne, joilla x. tai .y päätettä ei ole. 
Grassland_elop<-Grassland_elop %>% select(!(ends_with(".y")))

#Sarakejärjestyksen oikaisu
...
Cropland_mineral<-Cropland_mineral %>% select(all_of(Nimet))
Cropland_elop<-Cropland_elop %>% select(all_of(Nimet))

Grassland_mineral<-Grassland_mineral %>% select(all_of(Nimet))
Grassland_elop<-Grassland_elop %>% select(all_of(Nimet))





outersect(colnames(Cropland_mineral), colnames(Cropland_elop))
...
Cropland_yhdistetyt_paastot<-Cropland_yhdistetyt_paastot %>% select(all_of(Nimet))
...
outersect(colnames(Grassland_mineral), colnames(Grassland_elop))
...
Grassland_yhdistetyt_paastot<- Grassland_yhdistetyt_paastot %>% select(!(ends_with(".x"))) 
Grassland_yhdistetyt_paastot<- Grassland_yhdistetyt_paastot %>% select(!(ends_with(".y"))) 
...
library(here)
library(openxlsx)
write.xlsx(Tulos, here("Output/Yksinkertaistettu_intensiteettilaskenta/CO2_viljav.xlsx"), overwrite = T)
Tuotekertoimet_tallennus %>% saveWorkbook(here("Output/Yksinkertaistettu_intensiteettilaskenta/CO2_tuotekertoimet_viljav.xlsx"), overwrite = T)
saveWorkbook(Raivaus_paasto_CO2, file=here("Output/Yksinkertaistettu_intensiteettilaskenta/Raivauspaasto_CO2_viljav.xlsx"),overwrite = T)
saveWorkbook(Raivaamaton_paasto_CO2, file=here("Output/Yksinkertaistettu_intensiteettilaskenta/Raivaamaton_paasto_CO2_viljav.xlsx"),overwrite = T)
