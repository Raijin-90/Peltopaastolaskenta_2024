...
a<-a %>% pivot_longer(cols = 6:length(Cropland_korotettu_elop_raivio), names_to = "Tuotantosuunta", values_to = "N2O_crop_elop_raiv")
b<-b %>% pivot_longer(cols = 6:length(Grassland_korotettu_elop_raivio), names_to = "Tuotantosuunta", values_to = "N2O_grass_elop_raiv")
...
a<-a %>% pivot_longer(cols = 6:length(a), names_to = "Tuotantosuunta", values_to = "N2O_crop_elop")
b<-b %>% pivot_longer(cols = 6:length(b), names_to = "Tuotantosuunta", values_to = "N2O_grass_elop")
...
library(here)
library(openxlsx)
write.xlsx(Tulos, here("Output/Yksinkertaistettu_intensiteettilaskenta/N2O_viljav.xlsx"), overwrite = T)
Tuotekertoimet_tallennus %>% saveWorkbook(here("Output/Yksinkertaistettu_intensiteettilaskenta/N2O_tuotekertoimet_viljav.xlsx"), overwrite = T)
saveWorkbook(Raivaus_paasto_N2O, file=here("Output/Yksinkertaistettu_intensiteettilaskenta/Raivauspaasto_N2O_viljav.xlsx"),overwrite = T)
saveWorkbook(Raivaamaton_paasto_N2O, file=here("Output/Yksinkertaistettu_intensiteettilaskenta/Raivaamaton_paasto_N2O_viljav.xlsx"),overwrite = T)
