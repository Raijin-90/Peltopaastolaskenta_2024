...
#Koostetaan viljelyalasupplementti, jossa erotellaan tuotetyypit ja maalajit. Ei koske grasslandia, koska käytetään cropland-tuotteiden 
#intensiteettien vertailuun tuotteiden välillä. 
...
Cropland_mineraalimaa_alat<-Cropland_mineraalimaa_alat %>% select(1,2,5,6,7)
...
Cropland_mineraalimaa_alat_raivio<-Cropland_mineraalimaa_alat_raivio %>% select(1,2,5,6,7)
...
Mineraaliala_Cropland <-
  merge(
    Cropland_mineraalimaa_alat,
    Cropland_mineraalimaa_alat_raivio,
    by = c("Kasvikoodi", "Kasvinimi","Tuoteryhmä","Tuotantosuunta"), all=T
  )
...
Cropland_elop_alat<-Cropland_elop_alat %>% select(1,2,5,6,7)
...
Cropland_elop_alat_raivio<-Cropland_elop_alat_raivio %>%select(1,2,5,6,7)
...
Cropland <- merge(Mineraaliala_Cropland, Elop_ala_cropland, by=c("Kasvikoodi", "Kasvinimi","Tuoteryhmä","Tuotantosuunta"),  all=T)
...
z<-Cropland %>% group_by(Tuoteryhmä) %>% summarise(Viljeltyä_mineraalia_cropland = sum(mineraaliala_cropl),
...
z$MineraaliPros<-(z$Mineraalimaata_yhteensa_cropland/z$Croplandia_yhteensa)*100

z$ElopPros<-(z$Eloperaista_yhteensa_cropland/z$Croplandia_yhteensa)*100

write.xlsx(z, file=here("Output/Yksinkertaistettu_intensiteettilaskenta/viljavuus_viljelyalat_tuotteittain_uusi.xlsx"), overwrite = T)
